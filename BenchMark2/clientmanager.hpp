...
class ClientManager {
public:
  ClientManager(MyCoroutine::Schedule &schedule, int64_t client_count, std::string ip, int port, int64_t rate_limit)
    ...
    for (int64_t i = 0; i < client_count_; i++) {
      clients_[i] = new Client(schedule, ip, port, temp_rate_limit_);
    }
  }
  ~ClientManager() {
    for (int64_t i = 0; i < client_count_; i++) {
